...
import answerer.TextLine;
import nlp.StopWords;
import nlp.stemmer.EnglishStemmer;

/**
 * JUnit tests to test out Natural Language Processing (NPL) functionalities
 * included in "nlp" package.
 */
public class NLPTests {
	...
	public void shouldEliminateCorrectly() {
		...
		scriptMap.put("We have the most important feature", "important feature");
		scriptMap.put("I'm sorry, but I don't want to be an emperor", "i'm sorry don't want emperor");
		scriptMap.put(
				"Greed has poisoned men's souls, has barricaded the world with hate, has goose-stepped us into misery and bloodshed.",
				"greed poisoned men's souls barricaded world hate goose-stepped misery bloodshed");
		scriptMap.put("Tell me a sentence", "tell sentence");
		...
		for (String script : scriptMap.keySet()) {
			...
			Assert.assertTrue("Invalid elimination " + script, eliminated.equals(scriptMap.get(script)));
			System.out.println();
		}
		...
		long t = Performance.testTimeAvg(() -> {
			...
			StopWords.eliminateWords(randomScript); // Execute only, no
													// allocation
		}, trials);
	...
	public void shouldParseScriptLine() {
		...
		TextLine scriptLine = new TextLine(line);
